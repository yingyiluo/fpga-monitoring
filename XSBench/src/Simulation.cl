...
channel SearchContext SC_QUEUE1 __attribute__((depth(BUFFER_SIZE)));
...
__kernel void simulation(int lookups)
{
	...
	for( int i = 0; i < lookups; i++ )
	{
		...
		for (int n = 0; n < STAGE_NUM; n++) {
			cmid = cll+ ((cul - cll) >> 1);
			BSCacheUnion cache_data = bsc[cmid];
			cul = (cache_data.data.d > p_energy) ? cmid : cul;
			cll = (cache_data.data.d > p_energy) ? cll : cmid;
			ul = (cache_data.data.d > p_energy) ? cache_data.index : ul;
			ll = (cache_data.data.d > p_energy) ? ll : cache_data.index;
		}

		SearchContext sc = {p_energy, ll, ul, mat};
		write_channel_altera(SC_QUEUE1, sc);
    }
}

#include "GridSearch_pipe_grid_nounroll_local_ori.cl"
#include "CalculateXS_decoupled_grid_vector_op.cl"
