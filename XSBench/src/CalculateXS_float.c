...
void calculate_micro_xs(   double p_energy, int nuc, long n_isotopes,
	...
	else if( grid_type == UNIONIZED) // Unionized Energy Grid - we already know the index, no binary search needed.
	{
		...
		// we're not reading off the end of the nuclide's grid
	//	printf("nuc: %d, energy_idx_at_nuc: %d\n", nuc, energy_grid[idx].xs_ptrs[nuc]);
	...
	//printf("\ndata: %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f", low->energy, low->total_xs, low->elastic_xs, low->absorbtion_xs, low->fission_xs, low->nu_fission_xs, high->energy, high->total_xs, high->elastic_xs, high->absorbtion_xs, high->fission_xs, high->nu_fission_xs);	
...
void calculate_macro_xs( double p_energy, int mat, long n_isotopes,
	...
	//printf("host idx: %d\n", idx);
	// printf("mat: %d, p_energy: %f, idx: %ld\n", mat, p_energy, idx);
...
long grid_search( long n, double quarry, GridPoint * A)
{
	...
	//printf("host ul: %ld, ll: %ld, e: %lf\n", upperLimit, lowerLimit, A[examinationPoint].energy);	
	return lowerLimit;
}
