...
//	{	
		...
		float * xs = (float *) calloc(5, sizeof(float));
		...
		for( int i = 0; i < in.lookups; i++ )
		{
			...
			// in the "xs" array
			memcpy(xs, macro_xs_vector, 5*sizeof(float));
...
//			printf("Simulation: hash: %d\n", hash);
                	vhash_result[0] += hash % 1000;
			/*
			for(int k = 0; k < 5; k++) {
				vhash_result[i % 1024] += macro_xs_vector[k];
			}
			*/
			#endif
//		}
