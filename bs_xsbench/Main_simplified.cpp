...
// CL binary name
//const char *binary_prefix = "bs_unroll12";
//const char *binary_prefix = "bs_nounroll";
const char *binary_prefix = "bs_nounroll_chan_simplifed";
...
*/
	...
	status = clEnqueueWriteBuffer(queues[K_SIMULATION], d_energy, CL_TRUE, 0, n_iso_grid * sizeof(double), energy, 0, NULL, NULL);
	checkError(status, "Failed to enqueue write buffer.\n");
/*
...
*/
	...
	status = clSetKernelArg(kernels[K_SIMULATION], arg++, sizeof(cl_mem), &d_energy);
	checkError(status, "Failed to set arg 1");
	status = clSetKernelArg(kernels[K_SIMULATION], arg++, sizeof(cl_mem), &d_vhash);
	checkError(status, "Failed to set arg 1");
	...
	//start_debug(debug_kernel, debug_queue, 0);
	...
	// =====================================================================
	status = clEnqueueReadBuffer(queues[K_SIMULATION], d_vhash, CL_TRUE, 0, sizeof(unsigned long), vhash, 0, NULL, NULL);
