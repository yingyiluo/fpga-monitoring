...
*/
	...
	arg = 0;
	status = clSetKernelArg(kernels[K_READER], arg++, sizeof(int), &in.lookups);
	checkError(status, "Failed to set arg 0");
	status = clSetKernelArg(kernels[K_READER], arg++, sizeof(cl_mem), &d_vhash);
	checkError(status, "Failed to set arg 1");
	...
	// =====================================================================
	status = clEnqueueReadBuffer(queues[K_READER], d_vhash, CL_TRUE, 0, sizeof(unsigned long), vhash, 0, NULL, NULL);
